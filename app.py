...
# Instructions for the CSV input format
st.info(
    "Please upload a CSV file that includes a header with a 'text' column. "
    "Each row should contain a text entry that you wish to process."
)
...
st.markdown("""
This dashboard allows you to upload a CSV file containing a **text** column, process the data through cleaning, grammar correction, IBM Watson NLU analysis, POS tagging, and topic modeling, and then view various visualizations. Finally, you can download the processed CSV file.
""")
...
if uploaded_file is not None:
    try:
        ...
        if 'text' not in data.columns:
            ...
            data['cleaned_text'] = data['text'].apply(clean_text)
            st.subheader("Cleaned Text Sample")
            st.write(data[['text', 'cleaned_text']].head(3))
            
            # --- Step 2: Grammar Correction ---
            st.write("Performing Grammar Correction...")
            tqdm.pandas(desc="Grammar Correction")
            data['corrected_text'] = data['cleaned_text'].progress_apply(correct_grammar)
            st.subheader("Grammar Corrected Text Sample")
            st.write(data[['cleaned_text', 'corrected_text']].head(3))
            
            # --- Step 3: IBM Watson NLU Analysis ---
            st.write("Performing IBM Watson NLU Analysis (this may take a while)...")
            tqdm.pandas(desc="IBM NLU Analysis with Emotion")
            data['nlu_analysis'] = data['corrected_text'].progress_apply(analyze_with_nlu)
            
            # --- Step 4: POS Tagging ---
            st.write("Performing POS Tagging...")
            tqdm.pandas(desc="POS Tagging")
            data['pos_tags'] = data['corrected_text'].progress_apply(get_pos_tags)
            
            # Display a sample of POS tags and counts
            st.subheader("POS Tagging Sample")
            st.write(data[['corrected_text', 'pos_tags']].head(3))
            all_pos_tags = [tag for tags_list in data['pos_tags'] for word, tag in tags_list]
            pos_tag_counts = Counter(all_pos_tags)
            st.write("**Top 10 POS Tag Counts:**")
            st.write(pos_tag_counts.most_common(10))
            ...
            # --- Step 6: Sentiment and Emotion Extraction ---
            data['nlu_sentiment'] = data['nlu_analysis'].apply(get_nlu_sentiment_score)
            data['nlu_sentiment_category'] = data['nlu_sentiment'].apply(categorize_nlu_sentiment)
            data['emotion_scores'] = data['nlu_analysis'].apply(get_emotion_scores)
            for emotion in ['sadness', 'joy', 'fear', 'disgust', 'anger']:
            ...
            # --- Step 7: Save Processed Data and Download ---
            csv_buffer = io.StringIO()
            data.to_csv(csv_buffer, index=False)
            csv_data = csv_buffer.getvalue()
            # Encode CSV data to bytes for the download button
            csv_bytes = csv_data.encode('utf-8')
            st.download_button(
                label="Download Processed CSV",
                data=csv_bytes,
            ...
            # 1. Distribution of Text Length before and after Cleaning
            st.subheader("Text Length Distributions")
            fig, ax = plt.subplots(1, 2, figsize=(12, 6))
            sns.histplot(data['text'].str.len(), kde=True, bins=30, color='skyblue', ax=ax[0])
            ax[0].set_title('Original Text Length Distribution')
            ...
            # 2. Word Cloud of Original Text
            st.subheader("Word Cloud of Original Text")
            original_text_combined = ' '.join(data['text'])
            wordcloud_original = WordCloud(width=800, height=400, background_color='white').generate(original_text_combined)
            fig_wc1, ax_wc1 = plt.subplots(figsize=(10, 5))
            ax_wc1.imshow(wordcloud_original, interpolation='bilinear')
            ax_wc1.set_title('Word Cloud of Original Text')
            ax_wc1.axis('off')
            st.pyplot(fig_wc1)
            
            # 3. Word Cloud of Cleaned Text
            st.subheader("Word Cloud of Cleaned Text")
            cleaned_text_combined = ' '.join(data['cleaned_text'])
            wordcloud_cleaned = WordCloud(width=800, height=400, background_color='white').generate(cleaned_text_combined)
            fig_wc2, ax_wc2 = plt.subplots(figsize=(10, 5))
            ax_wc2.imshow(wordcloud_cleaned, interpolation='bilinear')
            ax_wc2.set_title('Word Cloud of Cleaned Text')
            ax_wc2.axis('off')
            st.pyplot(fig_wc2)
            
            # 4. Distribution of IBM NLU Sentiment Scores
            st.subheader("IBM NLU Sentiment Score Distribution")
            fig_sent, ax_sent = plt.subplots(figsize=(8, 6))
            sns.histplot(data['nlu_sentiment'].dropna(), kde=True, bins=30, color='green', ax=ax_sent)
            ax_sent.axvline(x=0, color='black', linestyle='--', label='Neutral Threshold')
            ...
            # 8. Word Cloud for Topic 0 (LDA)
            st.subheader("Word Cloud for LDA Topic 0")
            topic_id = 0  # Change this if you want to view a different topic
            topic_words = dict(lda_model.show_topic(topic_id, topn=50))
            wordcloud_topic = WordCloud(width=800, height=400, background_color='white').generate_from_frequencies(topic_words)
            fig_topic, ax_topic = plt.subplots(figsize=(10, 5))
            ax_topic.imshow(wordcloud_topic, interpolation='bilinear')
